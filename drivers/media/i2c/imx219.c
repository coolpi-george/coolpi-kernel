...
#define IMX219_DIGITAL_EXPOSURE_MIN	4
#define IMX219_DIGITAL_EXPOSURE_MAX	65535
#define IMX219_DIGITAL_EXPOSURE_STEP		1
#define IMX219_DIGITAL_EXPOSURE_DEFAULT		0x640
...
#define IMX219_XVCLK_FREQ_24M		24000000
...
static int reg_read(struct i2c_client *client, const u16 addr)
{
	...
	if (ret < 0) {
		dev_warn(&client->dev, "Reading register test %x from %x failed\n",
			 addr, client->addr);
		return ret;
	}
...
dev_err(&client->dev, "imx219 stream  000\n");

	if (!enable)
		return reg_write_table(client, stop);
dev_err(&client->dev, "imx219 stream  001\n");
	...
	if (ret)
		return ret;
dev_err(&client->dev, "imx219 stream  002\n");
...
static int imx219_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on)	{
		...
		msleep(5);
		if (!IS_ERR(priv->power_gpio))
			gpiod_direction_output(priv->power_gpio, 1);
		...
		msleep(5);
		usleep_range(10 * 1000, 20 * 1000);		
		ret = clk_set_rate(priv->clk, IMX219_XVCLK_FREQ_24M);
		if (ret < 0)
			dev_info(dev, "Failed to set xvclk rate\n");
		if (clk_get_rate(priv->clk) != IMX219_XVCLK_FREQ_24M){
			dev_info(dev, "xvclk mismatched\n");
		}else{
			dev_info(dev, "xvclk matched OK\n");
		}
...
static int imx219_ctrls_init(struct v4l2_subdev *sd)
{
	...
	exposure_def = (exposure_max < IMX219_DIGITAL_EXPOSURE_DEFAULT) ?
		exposure_max : IMX219_DIGITAL_EXPOSURE_DEFAULT;
	v4l2_ctrl_new_std(&priv->ctrl_handler, &imx219_ctrl_ops,
			  V4L2_CID_EXPOSURE,
			  IMX219_DIGITAL_EXPOSURE_MIN,
			  exposure_max, IMX219_DIGITAL_EXPOSURE_STEP,
			  exposure_def);
			...
			const struct i2c_device_id *did)
{
	...
	priv->reset_gpio = devm_gpiod_get(dev, "reset", GPIOD_ASIS);
	if (IS_ERR(priv->reset_gpio))
		dev_warn(dev, "Failed to get reset-gpios\n");
	priv->power_gpio = devm_gpiod_get(dev, "power", GPIOD_ASIS);
	if (IS_ERR(priv->power_gpio))
		dev_warn(dev, "Failed to get power-gpios\n");
	priv->pwdn_gpio = devm_gpiod_get(dev, "pwdn", GPIOD_ASIS);
	if (IS_ERR(priv->pwdn_gpio))
	...
	dev_info(dev, "imx219_video_probe\n");
	ret = imx219_video_probe(client);
	if (ret < 0)
		return ret;

	dev_info(dev, "imx219_ctrls_init\n");
