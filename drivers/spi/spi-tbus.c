...
#define N_SPI_MINORS			16	/* ... up to 256 */
...
static inline void spidev_mst_slave_ready_status(struct spidev_data *spidev, bool status)
{
...
static bool spidev_mst_check_slave_ready(struct spidev_data *spidev)
{
	return spidev->ready_status;
}

static int spidev_mst_wait_for_slave_ready(struct spidev_data *spidev,
					   u32 timeout_us)
{
	bool ready;
	int ret;
...
{
	if (device_property_match_string(dev, "compatible", "spitbus") < 0)
		return 0;
...
static const struct of_device_id spidev_dt_ids[] = {
	{ .compatible = "tianmao,spitbus", .data = &spidev_of_check },
	{},
};
...
static const struct acpi_device_id spidev_acpi_ids[] = {
	 ...
	 */
	{ "SPT0004", (kernel_ulong_t)&spidev_acpi_check },
	{},
};
...
static int spidev_probe(struct spi_device *spi)
{
	...
	status = devm_request_irq(&spi->dev, spidev->ready_irqnum, spidev_mst_slave_ready_interrupt,
...
static int __init spidev_init(void)
{
	...
	spidev_class = class_create(THIS_MODULE, "spitbus");
