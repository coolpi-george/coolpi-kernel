...
{
	   phy_write(phydev, 0x1e, 0xA00D);//?��D���??��led1
	   phy_write(phydev, 0x1f, 0x0026);//?��D�䨺y?Y
	   phy_write(phydev, 0x1e, 0xA00E);//?��D���??��led2
	   phy_write(phydev, 0x1f, 0x00D8);//?��D�䨺y?Y
       return 0;
}
...
stmmac_setup_dma_desc(struct stmmac_priv *priv, unsigned int mtu)
{
	...
	if (!dma_conf->dma_tx_size)
		dma_conf->dma_tx_size = dma_conf->dma_tx_size ? priv->plat->dma_tx_size :
					DMA_DEFAULT_TX_SIZE;

	if (!dma_conf->dma_rx_size)
		dma_conf->dma_rx_size = dma_conf->dma_rx_size ? priv->plat->dma_rx_size :
					DMA_DEFAULT_RX_SIZE;
