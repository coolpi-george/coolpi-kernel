				       ...
				       struct ethtool_drvinfo *drvinfo)
{
	 ...
	 */
	snpsver = FXGMAC_GET_REG_BITS(ver, MAC_VR_SNPSVER_POS,
				      MAC_VR_SNPSVER_LEN);
	devid = FXGMAC_GET_REG_BITS(ver, MAC_VR_DEVID_POS,
				    MAC_VR_DEVID_LEN);
	userver = FXGMAC_GET_REG_BITS(ver, MAC_VR_USERVER_POS,
				      MAC_VR_USERVER_LEN);
	FXGMAC_PR("xlgma: No userver (%x) here, snpsver (%x) should be 0x51\n", userver, snpsver);
					...
					u32 msglevel)
{
	...
	printk("fxmac, set msglvl from %08x to %08x\n", pdata->msg_enable, msglevel); 
	...
	*/
	if (hfunc)
	{
		...
		*hfunc = ETH_RSS_HASH_TOP;
		printk("fxmac, get_rxfh for hash function\n"); 
	}
	
	if (indir)
	{
		fxgmac_get_reta(pdata, indir);
		printk("fxmac, get_rxfh for indirection tab\n"); 
	}

	if (key)
	{
		memcpy(key, pdata->rss_key, fxgmac_get_rxfh_key_size(netdev));
		printk("fxmac, get_rxfh  for hash key\n"); 
	}
			  ...
			  const u8 *key, const u8 hfunc)
{
	...
	printk("fxmac, set_rxfh callin, indir=%lx, key=%lx, func=%02x\n", (unsigned long)indir, (unsigned long)key, hfunc); 
	...
	if (indir) {
		...
		i = i;
		printk("fxmac, set_rxfh, change of indirect talbe is not supported.\n");
		return -EINVAL;
#else
				   ...
				   struct ethtool_rxnfc *cmd)
{
	...
	printk ("fxgmac_get_rss_hash_opts, hw=%02x, %02x\n", reg_opt, pdata->rss_options);

	if(reg_opt != pdata->rss_options)
	{
		printk ("fxgmac_get_rss_hash_opts, warning, options are not consistent\n");
	}
			   ...
			   u32 *rule_locs)
{
	...
	case ETHTOOL_GRXRINGS:
		...
		ret = 0;
		printk("fxmac, get_rxnfc for rx ring cnt\n"); 
		break;
	case ETHTOOL_GRXCLSRLCNT:
		cmd->rule_cnt = 0;
		ret = 0;
		printk("fxmac, get_rxnfc for classify rule cnt\n"); 
		break;
	case ETHTOOL_GRXCLSRULE:
		printk("fxmac, get_rxnfc for classify rules\n"); 
	...
	case ETHTOOL_GRXCLSRLALL:
		...
		/*ret = ixgbe_get_ethtool_fdir_all(adapter, cmd,
						 (u32 *)rule_locs);
		*/
		printk("fxmac, get_rxnfc for classify both cnt and rules\n"); 
		break;
	case ETHTOOL_GRXFH:
		ret = fxgmac_get_rss_hash_opts(pdata, cmd);
		printk("fxmac, get_rxnfc for hash options\n"); 
		break;
	default:
		break;
	}
				  ...
				  struct ethtool_rxnfc *nfc)
{
	...
	printk("fxgmac_set_rss_hash_opt call in,nfc_data=%llx,cur opt=%x\n", nfc->data, pdata->rss_options);
	...
	if (rssopt != pdata->rss_options) {
		if ((rssopt & UDP_RSS_FLAGS) &&
			...
			printk("enabling UDP RSS: fragmented packets"
			       " may arrive out of order to the stack above\n");

		printk("rss option changed from %x to %x\n", pdata->rss_options, rssopt);
		pdata->rss_options = rssopt;
#if 1
...
static int fxgmac_set_rxnfc(struct net_device *dev, struct ethtool_rxnfc *cmd)
{
	...
	case ETHTOOL_SRXCLSRLINS:
		...
		printk("set_rxnfc for rx cls rule insert-n\\a\n");
		break;
	case ETHTOOL_SRXCLSRLDEL:
		//no support. rx classifier rule delete
		//ret = ixgbe_del_ethtool_fdir_entry(adapter, cmd);
		printk("set_rxnfc for rx cls rule del-n\\a\n");
		break;
	case ETHTOOL_SRXFH:
		printk("set_rxnfc for rx rss option\n");
						...
						struct ethtool_ringparam *ring)
{
	...
	printk("fxmac, get_ringparam callin\n"); 
			  ...
			  struct ethtool_wolinfo *wol)
{
	...
	*/
	...
	if (!(pdata->hw_feat.rwk) || !device_can_wakeup(/*pci_dev_to_dev*/(pdata->dev))) {
		printk("fxgmac get_wol, pci does not support wakeup\n"); 
		return;
	}
#if 0
...
#endif
	printk("fxmac, get_wol, 0x%x, 0x%x\n", wol->wolopts, pdata->wol); 
}
...
#endif
#endif
		printk("fxmac, set_wol, not supported wol options, 0x%x\n", wol->wolopts); 
		return -EOPNOTSUPP;
	}

	if (!(pdata->hw_feat.rwk)) {
		printk("fxmac, set_wol, hw wol feature is n/a\n"); 
...
/*
	if (wol->wolopts & WAKE_BCAST) {
		...
		printk("fxmac, set_wol, broadcast\n");
		fxgmac_pci_config_modify(struct pci_dev *pdev);
	}
*/	
#endif
	printk("fxmac, set_wol, opt=0x%x, 0x%x\n", wol->wolopts, pdata->wol); 
				    ...
				    struct ethtool_link_ksettings *cmd)
{
    ...
    u32 supported_link = 0xffffffff;
    u32 duplex, regval;

    fuxi_ephy_autoneg_ability_get(pdata, (unsigned int *) &supported_link);
...
#else
    clear_bit(ETHTOOL_LINK_MODE_Pause_BIT, cmd->link_modes.advertising);
    clear_bit(ETHTOOL_LINK_MODE_Asym_Pause_BIT, cmd->link_modes.advertising);
#endif
				    ...
				    const struct ethtool_link_ksettings *cmd)
{
	...
	printk("fxgmac_set_link_ksettings ok, speed=%08x, duplex = %d, autoneg = %d.\n", 
                                            pdata->phy_speed, pdata->phy_duplex, pdata->phy_autoeng);
				 ...
				 struct ethtool_pauseparam *pause)
{
	...
	printk("fxmac get_pauseparam done, rx=%d, tx=%d\n", pdata->rx_pause, pdata->tx_pause); 
}
				...
				struct ethtool_pauseparam *pause)
{
	...
	//printk("fxgmac set pause parameter, autoneg=%d\n", pause->autoneg);
	...
	*/
	printk("fxgmac set pause parameter, autoneg=%d, rx=%d, tx=%d\n", pause->autoneg, pause->rx_pause, pause->tx_pause);
...
#endif
	{
		//printk("fxgmac_ethtool_get_ethtool_stats, here\n");
			    ...
			    struct ethtool_test *eth_test, u64 *data)
{
	...
	if (eth_test->flags == ETH_TEST_FL_OFFLINE/*1, tx here*/) {
		...
		//printk("ethtool dbg:tx ring desc,num=%d,cur=%d\n", fxgmac_ethtool_test_len, cur_desc);
		...
		for(j=0/*, i = fxgmac_cur_tx_desc_start_idx*/; i < fxgmac_ethtool_test_len; i++){ 
			...
			//printk("ethtool dbg:desc val of %d=%u\n", i, temp_desc);
			...
			//printk("ethtool dbg:tx ring desc %d, %s\n", i, &fxgmac_gstrings_test[i][0]);
			j++;
		}
		fxgmac_dbg_ethtool_tx_desc_access = 0; //yzhang prevent access to desc both sides
