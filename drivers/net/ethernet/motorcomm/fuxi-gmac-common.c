...
static void fxgmac_default_config(struct fxgmac_pdata *pdata)
{
	...
	FXGMAC_PR("FXGMAC_DRV_NAME:%s, FXGMAC_DRV_VERSION:%s\n", FXGMAC_DRV_NAME, FXGMAC_DRV_VERSION);
}
...
static void fxgmac_init_all_ops(struct fxgmac_pdata *pdata)
{
	...
	FXGMAC_PR("register desc_ops and hw ops\n");
}
...
int fxgmac_init(struct fxgmac_pdata *pdata, bool save_private_reg)
{
    ...
    netdev->base_addr = (unsigned long)pdata->mac_regs - FUXI_MAC_REGS_OFFSET;
    ...
    FXGMAC_PR("num_online_cpus:%u, tx_ch_cnt:%u, tx_q_cnt:%u, tx_ring_count:%u\n",
    ...
    */
    ...
    FXGMAC_PR("rss_options:0x%x\n", pdata->rss_options);
    //FXGMAC_PR("TODO: RSS...need to write rss options to HW reg\n");
    ...
    FXGMAC_PR("fxgmac_init callout,ok.\n");
...
int fxgmac_drv_probe(struct device *dev, struct fxgmac_resources *res)
{
	...
	if(netif_msg_drv(pdata)) printk("fxgamc_drv_prob callout, netdev num_tx_q=%u\n", netdev->num_tx_queues);
...
err_free_netdev:
	free_netdev(netdev);
	printk("fxgamc_drv_prob callout with err \n");
		      ...
		      struct sk_buff *skb, bool tx_rx)
{
	...
	printk("%s packet of %d bytes\n",
		   (tx_rx ? "TX" : "RX"), skb->len);

	printk("Dst MAC addr: %pM\n", eth->h_dest);
	printk("Src MAC addr: %pM\n", eth->h_source);
	printk("Protocol: %#06hx\n", ntohs(eth->h_proto));
...
void fxgmac_get_all_hw_features(struct fxgmac_pdata *pdata)
{
	...
	if(netif_msg_drv(pdata)) printk ("check mac regs, get offset 0x110,ver=%#x,mac_regs=%p\n", readl(pdata->mac_regs + 0x110), pdata->mac_regs);
...
void fxgmac_print_all_hw_features(struct fxgmac_pdata *pdata)
{
	...
	FXGMAC_PR("=====================================================\n");
	FXGMAC_PR("\n");
	FXGMAC_PR("HW support following features,ver=%#x\n",pdata->hw_feat.version);
	FXGMAC_PR("\n");
	/* HW Feature Register0 */
	FXGMAC_PR("VLAN Hash Filter Selected                   : %s\n",
		  pdata->hw_feat.vlhash ? "YES" : "NO");
	FXGMAC_PR("SMA (MDIO) Interface                        : %s\n",
		  pdata->hw_feat.sma ? "YES" : "NO");
	FXGMAC_PR("PMT Remote Wake-up Packet Enable            : %s\n",
		  pdata->hw_feat.rwk ? "YES" : "NO");
	FXGMAC_PR("PMT Magic Packet Enable                     : %s\n",
		  pdata->hw_feat.mgk ? "YES" : "NO");
	FXGMAC_PR("RMON/MMC Module Enable                      : %s\n",
		  pdata->hw_feat.mmc ? "YES" : "NO");
	FXGMAC_PR("ARP Offload Enabled                         : %s\n",
		  pdata->hw_feat.aoe ? "YES" : "NO");
	FXGMAC_PR("IEEE 1588-2008 Timestamp Enabled            : %s\n",
		  pdata->hw_feat.ts ? "YES" : "NO");
	FXGMAC_PR("Energy Efficient Ethernet Enabled           : %s\n",
		  pdata->hw_feat.eee ? "YES" : "NO");
	FXGMAC_PR("Transmit Checksum Offload Enabled           : %s\n",
		  pdata->hw_feat.tx_coe ? "YES" : "NO");
	FXGMAC_PR("Receive Checksum Offload Enabled            : %s\n",
		  pdata->hw_feat.rx_coe ? "YES" : "NO");
	FXGMAC_PR("Additional MAC Addresses 1-31 Selected      : %s\n",
	...
	FXGMAC_PR("Timestamp System Time Source                : %s\n", str);

	FXGMAC_PR("Source Address or VLAN Insertion Enable     : %s\n",
	...
	FXGMAC_PR("MTL Receive FIFO Size                       : %s\n", str);
	...
	FXGMAC_PR("MTL Transmit FIFO Size                      : %s\n", str);

	FXGMAC_PR("IEEE 1588 High Word Register Enable         : %s\n",
		  pdata->hw_feat.adv_ts_hi ? "YES" : "NO");
	FXGMAC_PR("Address width                               : %u\n",
		  pdata->hw_feat.dma_width);
	FXGMAC_PR("DCB Feature Enable                          : %s\n",
		  pdata->hw_feat.dcb ? "YES" : "NO");
	FXGMAC_PR("Split Header Feature Enable                 : %s\n",
		  pdata->hw_feat.sph ? "YES" : "NO");
	FXGMAC_PR("TCP Segmentation Offload Enable             : %s\n",
		  pdata->hw_feat.tso ? "YES" : "NO");
	FXGMAC_PR("DMA Debug Registers Enabled                 : %s\n",
		  pdata->hw_feat.dma_debug ? "YES" : "NO");
	FXGMAC_PR("RSS Feature Enabled                   : %s\n",
		  pdata->hw_feat.rss ? "YES" : "NO");
	FXGMAC_PR("*TODO*Number of Traffic classes             : %u\n",
		  (pdata->hw_feat.tc_cnt));
	FXGMAC_PR("AV Feature Enabled                          : %s\n",
		  pdata->hw_feat.avsel ? "YES" : "NO");
	FXGMAC_PR("Rx Side Only AV Feature Enabled             : %s\n",
		  (pdata->hw_feat.ravsel ? "YES": "NO"));
	FXGMAC_PR("Hash Table Size                             : %u\n",
		  pdata->hw_feat.hash_table_size);
	FXGMAC_PR("Total number of L3 or L4 Filters            : %u\n",
		  pdata->hw_feat.l3l4_filter_num);

	/* HW Feature Register2 */
	FXGMAC_PR("Number of MTL Receive Queues                : %u\n",
		  pdata->hw_feat.rx_q_cnt);
	FXGMAC_PR("Number of MTL Transmit Queues               : %u\n",
		  pdata->hw_feat.tx_q_cnt);
	FXGMAC_PR("Number of DMA Receive Channels              : %u\n",
		  pdata->hw_feat.rx_ch_cnt);
	FXGMAC_PR("Number of DMA Transmit Channels             : %u\n",
		  pdata->hw_feat.tx_ch_cnt);
	...
	FXGMAC_PR("Number of PPS Outputs                       : %s\n", str);
	...
	FXGMAC_PR("Number of Auxiliary Snapshot Inputs         : %s", str);
