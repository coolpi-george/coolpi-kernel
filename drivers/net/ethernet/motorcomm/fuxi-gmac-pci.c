...
#define fxgmac_dbg_log(logstr, arg...)  \
        do { \
                ...
                if (!(IS_ERR(dbg_file))) { \
                        ...
                        /*printk("kernel write done, file size=%d, s=%s", (int)dbg_stat.size, log_buf)*/; \
                        set_fs(previous_fs); \
...
int fxgmac_dbg_log_init( void )
{
	...
	if (IS_ERR(dbg_file))
	{
		printk("File fxgmac_dbg.log open or created failed.\n");
...
int fxgmac_dbg_log_uninit( void )
{
	...
	if(IS_ERR(dbg_file)) 
	{
		printk("Invalid fd for file fxgmac_dbg.log, %p.\n", dbg_file);
		dbg_file= NULL;
		return PTR_ERR(dbg_file);
	}
...
int fxgmac_pci_config_modify(struct pci_dev *pdev)
{
	...
	if (pcie_cap_offset) {
		...
		if (ret)
		{
			printk(KERN_ERR "fxgmac_pci_config_modify read pci config space MSI cap. failed, %d\n", ret);
			return -EFAULT;
		}
	}
	...
	if (ret)
	{
		printk(KERN_ERR "fxgmac_pci_config_modify write pci config space MSI mask failed, %d\n", ret);
	}
...
static int fxgmac_probe(struct pci_dev *pcidev, const struct pci_device_id *id)
{
	...
	printk(KERN_INFO "fxgmac_probe callin\n");
	...
	for (i = 0; i <= PCI_STD_RESOURCE_END; i++) {
		...
		*/
		...
		if (ret)
		{
			printk(KERN_INFO "fxgmac_probe pcim_iomap_regions failed\n");
			return ret;
		}
		printk(KERN_INFO "fxgmac_probe iomap_region i=%#x,ret=%#x\n",(int)i,(int)ret);
		break;
	}
	...
	printk(KERN_INFO "fxgmac_probe res.addr=%p,val=%#x\n",res.addr,/*(int)(*((int*)(res.addr)))*/0);
	...
	printk("fxgmac_probe, MSIx, num of cpu=%d\n", vectors);
	if(vectors >= FXGMAC_MAX_DMA_CHANNELS) {
		...
		msix_entries = kcalloc(/*vectors 20220211*/req_vectors,
						...
						GFP_KERNEL);
		if (!msix_entries) {
			printk("fxgmac_probe, MSIx, kcalloc err for msix entries.\n");
		}else {
			...
			req_vectors = vectors;
		        do {
		                ...
		                if (rc < 0) {
					//failure
					//printk("fxgmac_probe, enable MSIx failed,%d.\n", rc);
					rc = 0; //stop the loop
...
#endif				
				printk("fxgmac_probe, enable MSIx ok, cpu=%d,vectors=%d.\n", vectors, req_vectors);
				pcidev_int_mode = FXGMAC_FLAG_MSIX_CAPABLE | FXGMAC_FLAG_MSIX_ENABLED;
				
			}else if (req_vectors){
				printk("fxgmac_probe, enable MSIx with only %d vector, while we need %d, rollback to MSI.\n", req_vectors, vectors);
				//roll back to msi
				pci_disable_msix(pcidev);
				kfree(msix_entries);
				msix_entries = NULL;			
			}else {
				printk("fxgmac_probe, enable MSIx failure and clear msix entries.\n");
				//roll back to msi
				kfree(msix_entries);
				msix_entries = NULL;			
			}
...
static int __fxgmac_shutdown(struct pci_dev *pdev, bool *enable_wake)
{
	...
	printk("fxpm,_fxgmac_shutdown, callin\n");
	fxgmac_dbg_log("fxpm,_fxgmac_shutdown, callin.\n");

	rtnl_lock();
    
        pdata->current_state = 0;

        hw_ops->close_phy_led(pdata);
	...
	if (retval) {
		printk("fxpm,_fxgmac_shutdown, save pci state failed.\n");
		return retval;
	}
#endif

	printk("fxpm,_fxgmac_shutdown, save pci state done.\n");
	...
	*enable_wake = !!wufc;
	...
	printk("fxpm,_fxgmac_shutdown callout, enable wake=%d.\n", *enable_wake);
...
static void fxgmac_shutdown(struct pci_dev *pdev)
{
	...
	printk("fxpm, fxgmac_shutdown callin\n");
	...
	printk("fxpm, fxgmac_shutdown callout, system power off=%d\n", (system_state == SYSTEM_POWER_OFF)? 1 : 0);
			 ...
			 pm_message_t __always_unused state)
{
	...
	printk("fxpm, fxgmac_suspend callin\n");
	...
	printk("fxpm, fxgmac_suspend callout to %s\n", wake ? "sleep" : "D3hot");
...
static int fxgmac_resume(struct pci_dev *pdev)
{
	...
	printk("fxpm, fxgmac_resume callin\n");
	...
	printk("fxpm, fxgmac_resume callout\n");
