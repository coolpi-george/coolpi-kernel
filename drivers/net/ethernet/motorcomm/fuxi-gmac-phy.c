...
int fuxi_ephy_autoneg_ability_get(struct fxgmac_pdata *pdata, unsigned int *cap_mask)
{
	...
	printk("fuxi_ephy_autoneg_ability_get, reg %d=0x%04x\n", reg, val);
	...
	printk("fuxi_ephy_autoneg_ability_get done, 0x%08x.\n", *cap_mask);

	return 0;

busy_exit:
	printk("fuxi_ephy_autoneg_ability_get exit due to ephy reg access fail.\n");
	
    return -1;
}

/* this function used to double check the speed. for fiber, to correct there is no 10M */
static int fuxi_ephy_adjust_status(u32 lport, int val, int is_utp, int* speed, int* duplex)
{
	int speed_mode;
	...
	*duplex = (val & BIT(FUXI_EPHY_DUPLEX_BIT)) >> FUXI_EPHY_DUPLEX_BIT;
	...
	//printk (KERN_INFO "8521 status adjust call out,regval=0x%04x,mode=%s,speed=%dm...\n", val,is_utp?"utp":"fiber", phydev->speed);
...
int fuxi_ephy_soft_reset(struct fxgmac_pdata *pdata)
{
	...
	do {
		ret = hw_ops->read_ephy_reg(pdata, REG_MII_BMCR, (unsigned int *)&val);
		busy--;
		//printk("fuxi_ephy_soft_reset, check busy=%d.\n", busy);
	}while((ret >= 0) && (0 != (val & 0x8000)) && (busy));

	if(0 == (val & 0x8000)) return 0; 

	printk("fuxi_ephy_soft_reset, timeout, busy=%d.\n", busy);
	return -EBUSY;
	
busy_exit:
	printk("fuxi_ephy_soft_reset exit due to ephy reg access fail.\n");
 ...
 */
int fuxi_ephy_status_get(struct fxgmac_pdata *pdata, int* speed, int* duplex, int* ret_link, int *media)
{
	struct fxgmac_hw_ops *hw_ops = &pdata->hw_ops;
        int ret;
        u16 reg;
	...
	if (link) {
		link_utp = 1;
		fuxi_ephy_adjust_status(0, val, 1, speed, duplex);
	} else {
		link_utp = 0;
	}
	...
	//printk (KERN_INFO "8614 read status call out,link=%d,linkmode=%d\n", ret_link, link_mode_8614[lport] );
	return 0;
	
busy_exit:
	printk("fuxi_ephy_status_get exit due to ephy reg access fail.\n");
...
/**
 * fuxi_phy_update_link - update the phy link status
 * @adapter: pointer to the device adapter structure
 **/
static void fuxi_phy_update_link(struct net_device *netdev)
{
	...
	{
        	printk("fuxi_phy link phy speed changed,%d->%d\n", pre_phy_speed, phy_speed);
	        switch(phy_speed){
        	case 2:
...
static void fuxi_phy_link_poll(struct timer_list *t)
#else
static void fuxi_phy_link_poll(unsigned long data)
#endif
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,15,0))
	...
	{
		printk("fuxi_phy_timer polling with NULL netdev %lx\n",(unsigned long)(pdata->netdev));
		return;
	}
...
#endif
	{
		...
		fuxi_phy_update_link(pdata->netdev);
	}else {
		printk("fuxi_phy_timer polling, powerstate changed, %ld, netdev=%lx, tm=%lx\n", pdata->powerstate, (unsigned long)(pdata->netdev), (unsigned long)&pdata->phy_poll_tm);
	}		

	//printk("fuxi_phy_timer polled,%d\n",cnt_polling);
}
...
void fuxi_phy_timer_resume(struct fxgmac_pdata *pdata)
{
	if(NULL == pdata->netdev)
	{
       	printk("fuxi_phy_timer_resume, failed due to NULL netdev %lx\n",(unsigned long)(pdata->netdev));
		return;
	}

	mod_timer(&pdata->phy_poll_tm,jiffies + HZ / 2);
	
	printk("fuxi_phy_timer_resume ok, fxgmac powerstate=%ld, netdev=%lx, tm=%lx\n", pdata->powerstate, (unsigned long)(pdata->netdev), (unsigned long)&pdata->phy_poll_tm);
}
...
int fuxi_phy_timer_init(struct fxgmac_pdata *pdata)
{
	...
	init_timer_key(&pdata->phy_poll_tm,NULL/*function*/,0/*flags*/,"fuxi_phy_link_update_timer"/*name*/,NULL/*class lock key*/);
        ...
        pdata->phy_poll_tm.function = (void *)(fuxi_phy_link_poll);
 #if (LINUX_VERSION_CODE < KERNEL_VERSION(4,15,0))
        pdata->phy_poll_tm.data = (unsigned long)pdata;
 #endif
        add_timer(&pdata->phy_poll_tm);

        printk("fuxi_phy_timer started, %lx\n", jiffies);
        return 0;
}
