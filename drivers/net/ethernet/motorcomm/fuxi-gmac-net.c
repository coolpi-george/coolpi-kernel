...
unsigned char * fxgmac_get_netdev_ip6addr(struct fxgmac_pdata *pdata, unsigned char *ipval, unsigned char *ip6addr_solicited, unsigned int ifa_flag)
{
	...
	if (ifa_flag & FXGMAC_NS_IFA_GLOBAL_UNICAST)
		printk ("%s FXGMAC_NS_IFA_GLOBAL_UNICAST is set, %x\n", __FUNCTION__, ifa_flag);

	if (ifa_flag & FXGMAC_NS_IFA_LOCAL_LINK)
		printk ("%s FXGMAC_NS_IFA_LOCAL_LINK is set, %x\n", __FUNCTION__, ifa_flag);
	...
	if (i6dev != NULL) {
		...
		list_for_each_entry(ifp, &i6dev->addr_list, if_list) {
			...
			if (((ifa_flag & FXGMAC_NS_IFA_GLOBAL_UNICAST) && (ifp->scope != IFA_LINK)) || ((ifa_flag & FXGMAC_NS_IFA_LOCAL_LINK) && (ifp->scope == IFA_LINK)/* &&
				...
				//printk("%s, netdev %s IPv6 local-link address %pI6\n",__FUNCTION__, netdev->name, addr_ip6);
				//printk("%s, netdev %s IPv6 solicited-node add %pI6\n",__FUNCTION__, netdev->name, addr_ip6_solicited);
				break;
			}
		}
	...
	if(err) printk("%s get ipv6 addr failed, use default.\n", __FUNCTION__);

	//printk("%s, netdev %s IPv6 local-link address %pI6\n",__FUNCTION__, netdev->name, addr_ip6);
	//printk("%s, netdev %s IPv6 solicited-node adr %pI6\n",__FUNCTION__, netdev->name, addr_ip6_solicited);
			...
			unsigned int count)
{
	...
	if (count > fxgmac_tx_avail_desc(ring)) {
		...
		if(netif_msg_tx_done(pdata)) printk("about stop tx q, ret BUSY\n");
			   ...
			   struct fxgmac_pkt_info *pkt_info)
{
	...
	if(default_msg_level & NETIF_MSG_TX_DONE){
		FXGMAC_PR("header_len=%u\n", pkt_info->header_len);
		FXGMAC_PR("tcp_header_len=%u, tcp_payload_len=%u\n",
			       ...
			       struct fxgmac_pkt_info *pkt_info)
{
	...
	if(netif_msg_tx_done(pdata)) 
		printk ("fxgmac_prep_tx_pkt callin,pkt desc cnt=%d,skb len=%d, skbheadlen=%d\n", pkt_info->desc_count, skb->len, skb_headlen(skb));
	...
	if (fxgmac_is_tso(skb)) {
		...
		pkt_info->attributes = FXGMAC_SET_REG_BITS(
					...
					1);
		if(netif_msg_tx_done(pdata)) printk ("fxgmac_prep_tx_pkt,tso, pkt desc cnt=%d\n", pkt_info->desc_count);
	...
	if (skb_vlan_tag_present(skb)) {
		...
		pkt_info->attributes = FXGMAC_SET_REG_BITS(
					...
					1);
		if(netif_msg_tx_done(pdata)) printk ("fxgmac_prep_tx_pkt,VLAN, pkt desc cnt=%d,vlan=0x%04x\n", pkt_info->desc_count, skb_vlan_tag_get(skb));
	}
	...
	if(netif_msg_tx_done(pdata)) 
		printk ("fxgmac_prep_tx_pkt callout,pkt desc cnt=%d,skb len=%d, skbheadlen=%d,frags=%d\n", pkt_info->desc_count, skb->len, skb_headlen(skb), skb_shinfo(skb)->nr_frags);
}
...
#endif
{
	...
	if(hw_reg_cur == channel->tx_hang_hw_cur) {
		...
		if(!pdata->tx_hang_restart_queuing) {
			...
			printk("tx_hang_timer_handler: restart scheduled, at desc %u, queuing=%u.\n", channel->tx_hang_hw_cur, pdata->tx_hang_restart_queuing);	
...
static void fxgmac_napi_enable(struct fxgmac_pdata *pdata, unsigned int add)
{
	...
	if (pdata->per_channel_irq) {
		...
		for (i = 0; i < pdata->channel_count; i++, channel++) {
			...
			if(netif_msg_drv(pdata)) printk("napi_enable, msix ch%d napi enabled done,add=%d\n", i, add);
		}
	} else {
		if (add)
...
static void fxgmac_napi_disable(struct fxgmac_pdata *pdata, unsigned int del)
{
	...
	if (pdata->per_channel_irq) {
		...
		for (i = 0; i < pdata->channel_count; i++, channel++) {
			...
			if(netif_msg_drv(pdata)) printk("napi_disable, msix ch%d napi disabled done,del=%d\n", i, del);
		}
	} else {
...
static int fxgmac_request_irqs(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->channel_count; i++, channel++) {
		...
		if(FXGMAC_IS_CHANNEL_WITH_TX_IRQ(i)) {
			...
			if (ret) {
				printk("fxgmac_req_irqs, err with MSIx irq request for ch %d tx, ret=%d\n", i, ret);
			...
			if(netif_msg_drv(pdata)) printk("fxgmac_req_irqs, MSIx irq_tx request ok, ch=%d, irq=%d,%s\n", i, channel->dma_irq_tx, channel->dma_irq_name_tx);
		}
#endif
	...
	if(netif_msg_drv(pdata)) printk("fxgmac_req_irqs, MSIx irq request ok, total=%d,%d~%d\n", i, (pdata->channel_head)[0].dma_irq, (pdata->channel_head)[i-1].dma_irq);
	return 0;

err_irq:
	printk("fxgmac_req_irqs, err with MSIx irq request at %d, ret=%d\n", i, ret);
	/* Using an unsigned int, 'i' will go to UINT_MAX and exit */
...
static void fxgmac_free_irqs(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->channel_count; i++, channel++) {
		...
		if(FXGMAC_IS_CHANNEL_WITH_TX_IRQ(i)) {
			channel->flag_napi_tx = 0;
			devm_free_irq(pdata->dev, channel->dma_irq_tx, channel);
			if(netif_msg_drv(pdata)) printk("fxgmac_free_irqs, MSIx irq_tx clear done, ch=%d\n", i);
		}
#endif				
		devm_free_irq(pdata->dev, channel->dma_irq, channel);
	}
	
	if(netif_msg_drv(pdata)) printk("fxgmac_free_irqs, MSIx irq clear done, total=%d\n", i);
}
...
static int fxgmac_start(struct fxgmac_pdata *pdata)
{
	...
	if(netif_msg_drv(pdata)) printk("fxgmac start callin here.\n");
...
err_napi:
	...
	hw_ops->exit(pdata);
	printk("fxgmac start callout with irq err.\n");
	return ret;
}
...
void fxgmac_net_powerup(struct fxgmac_pdata *pdata)
{
    ...
    if(netif_msg_drv(pdata)) printk("fxgmac_net_powerup callin\n");
...
void fxgmac_net_powerdown(struct fxgmac_pdata *pdata, unsigned int wol)
{
    ...
    if(netif_msg_drv(pdata)) printk("fxgmac_net_powerdown callin here.\n");

    /* signal that we are down to the interrupt handler */
    if (__test_and_set_bit(FXGMAC_POWER_STATE_DOWN, &pdata->powerstate))
        return; /* do nothing if already down */

    if(netif_msg_drv(pdata)) printk("fxgmac_net_powerdown continue with down process.\n");
    ...
    fxgmac_free_rx_data(pdata);
#endif
    if(netif_msg_drv(pdata)) printk("fxgmac_net_powerdown callout, powerstate=%ld.\n", pdata->powerstate);
}
...
static int fxgmac_open(struct net_device *netdev)
{
	...
	u32 phy_speed = 2; //default to 1000m
#endif
	if(netif_msg_drv(pdata)) printk("fxgmac_open callin\n");
	...
	//printk("fxgmac_open, b4 calc rx buf size, mtu,min,max=%d,%d,%d.\n", netdev->mtu, netdev->min_mtu, netdev->max_mtu);
	...
	//fxgmac_act_phy_link(pdata);
	fuxi_phy_timer_init(pdata);

#if 0 /* yzhand debug to disable tx temporaly */
                netif_tx_stop_all_queues(netdev);
		printk("for debug and disable tx....\n");
#endif
	if(netif_msg_drv(pdata)) printk("fxgmac_open callout\n");

	return 0;

err_channels_and_rings:	
	desc_ops->free_channels_and_rings(pdata);
	printk("fxgmac_open callout with channel alloc err\n");
	return ret;
}
...
static int fxgmac_close(struct net_device *netdev)
{
	...
	if(netif_msg_drv(pdata)) printk("fxgmac_close callin\n");
	...
	/*clean timer*/
	fuxi_phy_timer_destroy(pdata);
	
	if(netif_msg_drv(pdata)) printk("fxgmac_close callout\n");
...
static int fxgmac_xmit(struct sk_buff *skb, struct net_device *netdev)
{
	...
	//yzhang disabled 
	if(netif_msg_tx_done(pdata)) printk("xmit callin, skb->len=%d,q=%d\n", skb->len, skb->queue_mapping);
	...
	if (ret)
	{
		printk("xmit, no enoughp q, stop,ret=%d\n", ret);
		return ret;
	}
	...
	if (ret) {
		...
		printk("dev_xmit,tx err for TSO\n");
		dev_kfree_skb_any(skb);
		return ret;
	}
	...
	if (!desc_ops->map_tx_skb(channel, skb)) {
		dev_kfree_skb_any(skb);
		printk("xmit, map tx skb err\n");
		return NETDEV_TX_OK;
	}

	/* Report on the actual number of bytes (to be) sent */
	netdev_tx_sent_queue(txq, tx_pkt_info->tx_bytes);
	if(netif_msg_tx_done(pdata)) printk("xmit,before hw_xmit, byte len=%d,txtimeout=%lu\n", tx_pkt_info->tx_bytes, txq->trans_timeout);
	
	/* Configure required descriptor fields for transmission */
	hw_ops->dev_xmit(channel);
#if FXGMAC_DUMMY_TX_DEBUG
	printk("tx hw_ops->dev_xmit ok\n");
#endif
	if (netif_msg_pktdata(pdata))
		fxgmac_dbg_pkt(netdev, skb, true);

	/* Stop the queue in advance if there may not be enough descriptors */
	ret=fxgmac_maybe_stop_tx_queue(channel, ring, FXGMAC_TX_MAX_DESC_NR);
	if (ret)
	{
		printk("xmit, 2nd-no enoughp q, stop,ret=%d\n", ret);
	}
	return NETDEV_TX_OK;
}
...
#endif
	{
		//printk("get_stats64, ndo op, callin\n");
...
static int fxgmac_set_mac_address(struct net_device *netdev, void *addr)
{
	...
	printk("fxgmac,set mac addr to %02x:%02x:%02x:%02x:%02x:%02x\n",netdev->dev_addr[0], netdev->dev_addr[1], netdev->dev_addr[2],
			netdev->dev_addr[3], netdev->dev_addr[4], netdev->dev_addr[5]);
	return 0;
}

/* for this function, pls use below shell cmd to test
			...
			struct ifreq *ifreq, int cmd)
{
	...
	printk("fxgmac,ioctl, cmd=%x.\n", cmd);
	if (!netif_running(netdev))
		return -ENODEV;

	if((SIOCGMIIREG == cmd) || (SIOCSMIIREG == cmd))
		printk("fxgmac mii phy %d reg %d access needed, tbd.\n", mii->phy_id & 0x1f, mii->reg_num);
...
static int fxgmac_change_mtu(struct net_device *netdev, int mtu)
{
	...
	printk("fxgmac,set MTU from %d to %d. min, max=(%d,%d)\n",old_mtu, netdev->mtu, netdev->min_mtu, netdev->max_mtu);
#else
        printk("fxgmac,set MTU from %d to %d.\n",old_mtu, netdev->mtu);
#endif
			       ...
			       netdev_features_t features)
{
	...
	printk("fxgmac,set features done,%llx\n", (u64)features);
	return 0;
}
...
static void fxgmac_rx_refresh(struct fxgmac_channel *channel)
{
	...
	struct fxgmac_desc_ops *desc_ops;
	struct fxgmac_hw_ops *hw_ops;
	unsigned int temp_cur, temp_dirty,temp_num_dirty_unmapped=0, rx_tail_idx_update_to = 0;
	...
	if(netif_msg_rx_status(pdata)) 
		printk("rx_refresh..,tmp_cur=%d,tmp_dirty=%d,update_idx=%d,dirty=%d, unmapped_desc=%d\n", temp_cur, temp_dirty, 
...
static int fxgmac_tx_poll(struct fxgmac_channel *channel)
{
	...
	if (!ring){
		if(netif_msg_tx_done(pdata) && (channel->queue_index < pdata->tx_q_count)) printk("tx_poll, null point to ring %d\n", channel->queue_index);
		return 0;
	}
	if((ring->cur != ring->dirty) && (netif_msg_tx_done(pdata)))
		printk("tx_poll callin, ring_cur=%d,ring_dirty=%d,qIdx=%d\n", ring->cur, ring->dirty, channel->queue_index);
	...
	while ((processed < FXGMAC_TX_DESC_MAX_PROC) &&
		...
		if (!hw_ops->tx_complete(dma_desc)) {
			...
			if (change_cnt > 2)
			{
				...
				printk("after complete check, cur=%d, dirty=%d,qIdx=%d, hw desc cur=%#x, pre=%#x\n", ring->cur, ring->dirty, channel->queue_index, 
					reg_cur, reg_cur_pre);

				if((ring->cur > ring->dirty) && ((ring->cur - ring->dirty) > FXGMAC_HANG_THRESHOLD) ) {
					printk("after complete check warning..., too many TBD occupied by HW, 0xdbbb, %d.\n", (ring->cur - ring->dirty));
					...
					printk("after complete check warning..., too many TBD occupied by HW, 0xdb00, %d.\n", (ring->cur + (ring->dma_desc_count - ring->dirty)));	
	...
	if(netif_msg_tx_done(pdata)) printk("tx_poll: before status check of tx poll timer for complete check, tx timer usecs=%u, active=%u.\n", pdata->tx_usecs, channel->tx_timer_active);
	//spin_lock(&pdata->txpoll_lock);
    if ((ring->dirty != cur) && (pdata->tx_usecs && !channel->tx_timer_active)) {
        channel->tx_timer_active = 1;
        mod_timer(&channel->tx_timer,
                  jiffies + usecs_to_jiffies(pdata->tx_usecs));
		if(netif_msg_tx_done(pdata)) printk("tx_poll: continue with tx poll timer for complete check.\n");
    }
	//spin_unlock(&pdata->txpoll_lock);
	...
	if(netif_msg_tx_done(pdata)){
		FXGMAC_PR("tx_poll callout, processed=%d\n", processed);
	}
...
read_again:
		...
		//printk("rx_poll, dbg for ethtool: cur_ch=%u, cur_desc=%d\n", fxgmac_cur_rx_ch_polling, ring->cur);
		...
		//printk("rx_poll received=%d,incomplete=%d,context=%d\n", received, incomplete, context);
		...
		if (!context) {
			...
			if (dma_desc_len && !skb) {
				...
				if (!skb)
					error = 1;
				//printk("rx_poll:not context and create skb,at%#x,error=%d,desc_len=%u\n", (unsigned int)skb, error,dma_desc_len);
				...
				skb_add_rx_frag(
					...
					dma_desc_len,
					desc_data->rx.buf.dma_len);
				printk("rx_poll: rx frag and add to skb, dma_desc_len=%d\n", dma_desc_len);
				desc_data->rx.buf.pa.pages = NULL;
			}
		}
...
#else
			printk("rx_poll: pkt len %d exceeds MTU %d\n", skb->len, max_len);
#endif
			dev_kfree_skb(skb);
			goto next_packet;
		}
		...
		skb_checksum_none_assert(skb);
                if (netdev->features & NETIF_F_RXCSUM)
                {
                        ...
                        if(netif_msg_rx_status(pdata))
                                printk("ip payload error status:%d,ip head error status:%d\n",ipce, iphe);
                }
		...
		if (FXGMAC_GET_REG_BITS(pkt_info->attributes,
			...
			if(netif_msg_rx_status(pdata)) printk("fxgmac_rx_poll,pkt vlan=%d,total=%llu\n", pkt_info->vlan_ctag, pdata->stats.rx_vlan_packets);
		}
...
#define TEST_MAC_HEAD_LEN        14
                if(fxgmac_test_tso_flag)
                {
                    /* tso test */
                    if(fxgmac_test_tso_seg_num == 1)
                    {
                        /* last segment */
                        if(fxgmac_test_last_tso_len == skb->len + TEST_MAC_HEAD_LEN)
                        {
                            /* receive last segment, reset flag */
                            fxgmac_test_tso_flag = false;
                            fxgmac_test_tso_seg_num = 0;
                            fxgmac_test_packet_len = 0;
                            fxgmac_test_last_tso_len = 0;
        
                            /* process packet */
                            if((fxgmac_test_skb_arr_in_index + 1) % MAX_DBG_TEST_PKT != fxgmac_test_skb_arr_out_index){
                                struct sk_buff *tmpskb = skb_copy(skb, GFP_ATOMIC);
                                skb_push(tmpskb, TEST_MAC_HEAD_LEN);
        
                                fxgmac_test_skb_array[fxgmac_test_skb_arr_in_index] = tmpskb;
                                fxgmac_test_skb_arr_in_index = (fxgmac_test_skb_arr_in_index + 1) % MAX_DBG_TEST_PKT;
                            }
                            else{
                                printk("loopback test buffer is full.");
                            }
                        }
                    }
                    else /* non last segment */
                    {
                        if(fxgmac_test_packet_len == skb->len + TEST_MAC_HEAD_LEN){
                            /* receive a segment */
                ...
                else if(fxgmac_test_packet_len != 0)
                {
...
#endif
		cnt_yzhang_dbg_skb++; 
		//printk("rx_poll, napi_gro_rece called, and abt add skb cnt=%d\n", cnt_yzhang_dbg_skb);
...
next_packet:
	...
	//yzhang comment out FXGMAC_PR("rx_poll callout, packet_count = %d\n", packet_count);
...
static int fxgmac_one_poll(struct napi_struct *napi, int budget)
{
	...
	//FXGMAC_PR("one_poll %d callout,received = %d\n", channel->dma_irq, processed);
...
static int fxgmac_all_poll(struct napi_struct *napi, int budget)
{
	...
	//yzhang comment out
	if(netif_msg_rx_status(pdata)){
		FXGMAC_PR("rx all_poll callin budget=%d\n", budget);
	}
	...
	if((processed) && (netif_msg_rx_status(pdata))) { //yzhang for debug
		FXGMAC_PR("rx all_poll callout received = %d\n", processed);
	}
