...
obj-$(CONFIG_AIC8800_USB_WLAN_SUPPORT) += aic8800_fdrv/
...
install:
	mkdir -p $(MODDESTDIR)
	install -p -m 644 aic_load_fw/aic_load_fw_usb.ko  $(MODDESTDIR)/
	install -p -m 644 aic8800_fdrv/aic8800_fdrv_usb.ko  $(MODDESTDIR)/
	/sbin/depmod -a ${KVER}

uninstall:
	rm -rfv $(MODDESTDIR)/aic_load_fw_usb.ko
	rm -rfv $(MODDESTDIR)/aic8800_fdrv_usb.ko
	/sbin/depmod -a ${KVER}
