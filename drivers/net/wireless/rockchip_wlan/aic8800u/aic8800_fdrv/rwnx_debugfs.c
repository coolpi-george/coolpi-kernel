...
static ssize_t rwnx_dbgfs_trx_param_write(struct file *file,
			const char __user *user_buf,
			size_t count, loff_t *ppos)
{
	struct rwnx_hw *priv = file->private_data;
	...
	buf[len] = '\0';
	ret = sscanf(buf, "%x,%x,%x,%x,%x", &addr[0], &addr[1], &addr[2], &addr[3],&retry_cnt);

	if(ret != 5) {
		printk("param error\n");
	} else {
		printk("0x%x 0x%x 0x%x 0x%x 0x%x ", addr[0],  addr[1], addr[2], addr[3], retry_cnt);
		ret = rwnx_send_vendor_trx_param_req(priv, addr, vif_idx, retry_cnt);
		if(ret) {
			printk("rwnx_send_vendor_trx_param_req fail: %x\n", ret);
		}
	}
...
DEBUGFS_WRITE_FILE_OPS(trx_param);
...
int rwnx_dbgfs_register(struct rwnx_hw *rwnx_hw, const char *name)
{
	...
	DEBUGFS_ADD_FILE(trx_param, dir_drv,S_IWUSR);
