    ...
    if (jd9366t_GetSectionFirstValueStatus(50) == true) {
        ...
        if (ReCode < 0) {
            ...
            g_jd9366t_chip_info.dsram_host_addr.mpap_pw_sync =
                dsram_info_content[JD9366T_DSRAM_HOST].info_content_addr + 74;
#endif
            g_jd9366t_chip_info.dsram_host_addr.data_output_v2 =
                dsram_info_content[JD9366T_DSRAM_HOST].info_content_addr + 86;
...
static int jd9366t_HostWriteFlash(uint32_t WriteAddr, uint8_t *pFileData, uint32_t FileSize)
{
    ...
    do {
        ...
        /* 13-0. Clear PorInit, if upgrade succss */
        if (ReCode >= 0) {
            jd9366t_PorInit();
        }
...
static uint8_t module_jd9366t_DbicReg_read(uint8_t page, uint8_t cmd, uint8_t *rpar, uint8_t rpar_len, uint32_t offset)
{
    uint8_t i;
...
static void module_jd9366t_mutual_data_set(uint8_t data_type)
{
    ...
    if (pjadard_ts_data->debug_diag_apk_enable) {
        status[0] = 0x55;
        status[1] = 0xAA;
        jd9366t_WriteRegMulti(g_jd9366t_chip_info.dsram_host_addr.data_output_v2, status, sizeof(status));
    } else {
        status[0] = 0x00;
        status[1] = 0x00;
        jd9366t_WriteRegMulti(g_jd9366t_chip_info.dsram_host_addr.data_output_v2, status, sizeof(status));
...
static int module_jd9366t_read_mutual_data(uint8_t *rdata, uint16_t rlen)
{
    int ReCode;

    if (g_jd9366t_chip_info.esram_info_content_addr.output_buf > g_jd9366t_chip_info.esram_info_content_addr.coordinate_report) {
        ReCode = jd9366t_ReadRegMulti(g_jd9366t_chip_info.esram_info_content_addr.coordinate_report, rdata, rlen);
        if (ReCode >= 0)
...
void module_jd9366t_set_earphone_mode(unsigned int enable)
{
	uint8_t wdata[JD_TWO_SIZE] = {0x00, 0x00};
	if (enable) {
		wdata[0] = 1;
...
static void jd9366t_func_reinit(void)
{
    ...
    g_module_fp.fp_read_mutual_data     = module_jd9366t_read_mutual_data;
...
static bool module_jd9366t_chip_detect(void)
{
    ...
    if (g_common_variable.dbi_dd_reg_mode != JD_DDREG_MODE_0) {
        JD_E("%s:Don't support this mode ,dbi_dd_reg_mode %02X not equal %02X\n",
            __func__, g_common_variable.dbi_dd_reg_mode, JD_DDREG_MODE_0);
        return false;
    }
